    ...
    public class EmailService : IEmailService
    {
        ...
        public async Task<bool> SendEmailAsync(string email, string message)
        {
            try
            {
                ...
                mimeMessage.From.Add(new MailboxAddress("StudentHub System", emailSettings["Email"]));
                mimeMessage.To.Add(MailboxAddress.Parse(email));
                mimeMessage.Subject = "StudentHub - Mã xác thực OTP";
                mimeMessage.Body = new TextPart(TextFormat.Html) { Text = $@"
                    <div style='font-family: Arial, sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px; max-width: 500px; margin: auto;'>
                        <h2 style='color: #FF9800; text-align: center;'>Xác thực OTP - StudentHub</h2>
                        <p>Chào bạn,</p>
